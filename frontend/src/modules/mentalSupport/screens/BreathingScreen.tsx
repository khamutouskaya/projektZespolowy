import {
  View,
  StyleSheet,
  SectionList,
  Pressable,
  Text,
  Image,
  Linking,
} from "react-native";

import { useState, useRef, useEffect } from "react";
import { setAudioModeAsync } from "expo-audio";
import { useVideoPlayer, VideoView } from "expo-video";

import { colors } from "@/shared/theme/colors";
import { typography } from "@/shared/theme/typography";
import { spacing } from "@/shared/theme/spacing";

import LayoutContainer from "@/shared/layout/LayoutContainer";
import Header from "../components/shared/Header";
import { ScrollView } from "react-native";

// 🔥 фейковые видео
const sections = [
  {
    title: "Na sen",
    data: [
      {
        id: "2",
        title: "Przenieś się do innego świata",
        videoUrl:
          "https://res.cloudinary.com/dxsukcxzj/video/upload/v1774449689/Przenie%C5%9B_si%C4%99_do_INNEGO_%C5%9AWIATA_Medytacja_na_SEN_-_Chod%C5%BA_na_s%C5%82%C3%B3wko_720p_h264_def81g.mp4",
        thumbnail:
          "https://res.cloudinary.com/dxsukcxzj/image/upload/v1774465192/palce-med1_efhnyv.jpg",
      },
      {
        id: "3",
        title: "Wyłącz myśli i zaśnij",
        videoUrl:
          "https://res.cloudinary.com/dxsukcxzj/video/upload/v1774464144/Wieczorna_Medytacja_na_Sen_loqg0y.mp4",
        thumbnail:
          "https://res.cloudinary.com/dxsukcxzj/image/upload/v1774464148/Poranna_medytacja_w_lesie_dciaa7.png",
      },
      {
        id: "4",
        title: "Głęboki sen od pierwszej minuty",
        videoUrl:
          "https://res.cloudinary.com/dxsukcxzj/video/upload/v1774464182/Medytacja_na_g%C5%82%C4%99boki_relaks_lub_sen_oddech_przeponowy_12_min_prowadzenia_bez_wybudzania_jnxipc.mp4",
        thumbnail:
          "https://res.cloudinary.com/dxsukcxzj/image/upload/v1774464472/0e34170b9a01873d26d78a98297c6544_bhmmaz.jpg",
      },
      {
        id: "1",
        title: "Relaksacja z Intencją Miłości i Spokoju",
        videoUrl:
          "https://res.cloudinary.com/dxsukcxzj/video/upload/v1774437381/Metoda_oddechowa_Wima_Hofa_po_polsku_Poziom_I_-_Chod%C5%BA_na_s%C5%82%C3%B3wko_1080p_h264_bkozdq.mp4",
        thumbnail:
          "https://res.cloudinary.com/dxsukcxzj/image/upload/v1774462958/casa-tranquila-sala-de-estar-aconchegante-com-plantas-de-casa-conceito-sem-estresse-bela-mulher-de-30-anos-relaxando-em-um-sofa-confortavel_505557-31739_puztht.jpg",
      },
      {
        id: "5",
        title: "Wyłącz głowę przed snem",
        videoUrl:
          "https://res.cloudinary.com/dxsukcxzj/video/upload/v1774466826/Oddech_na_sen_rytm_4_8_10_min_SOMA_Breath_1_llgwjp.mp4",
        thumbnail:
          "https://res.cloudinary.com/dxsukcxzj/image/upload/v1774449941/360_F_597640752_zRM6XcUtquOp3TXiCbln7zx2qbjsWDRx_dqztuo.jpg",
      },
    ],
  },
  {
    title: "Na stres",
    data: [
      {
        id: "6",
        title: "Medytacja na lęk i stres",
        videoUrl:
          "https://res.cloudinary.com/dxsukcxzj/video/upload/v1774466840/20-minutowa_medytacja_prowadzona_na_l%C4%99k_i_stres_ivbxsu.mp4",
        thumbnail:
          "https://res.cloudinary.com/dxsukcxzj/image/upload/v1774531067/360_F_289706378_ST1xwYE1T31kznsSuLFEFZ4WiQ2EDHEV_zutn6c.jpg",
      },
      {
        id: "7",
        title: "Uspokój stres",
        videoUrl:
          "https://res.cloudinary.com/dxsukcxzj/video/upload/v1774467201/Medytacja_na_stres_l%C4%99k_i_niepok%C3%B3j_15_minut_g%C5%82%C4%99bokiego_wyciszenia_dbajosiebie_afirmacje_tdrvqh.mp4",
        thumbnail:
          "https://res.cloudinary.com/dxsukcxzj/image/upload/v1774467468/1549091181_24_wgw2ax.jpg",
      },
      {
        id: "8",
        title: "Obniż poziom kortyzolu",
        videoUrl:
          "https://res.cloudinary.com/dxsukcxzj/video/upload/v1774467403/Obni%C5%BC_poziom_KORTYZOLU_Medytacja_na_stres_i_l%C4%99k_ruaq2w.mp4",
        thumbnail:
          "https://res.cloudinary.com/dxsukcxzj/image/upload/v1774467777/ai-generated-beautiful-nature-mountain-scenery-professionalgraphy-photo_ukhtmv.jpg",
      },
      {
        id: "9",
        title: "Relaksacja Jacobsona",
        videoUrl:
          "https://res.cloudinary.com/dxsukcxzj/video/upload/v1774467693/Relaksacja_na_Sen_i_na_Stres_golgda.mp4",
        thumbnail:
          "https://res.cloudinary.com/dxsukcxzj/image/upload/v1774467639/420a73102444cf9485a75d390fafda7f_o9kkuy.jpg",
      },
      {
        id: "10",
        title: "Odpuść wszystko ",
        videoUrl:
          "https://res.cloudinary.com/dxsukcxzj/video/upload/v1774467752/Relaksacja_Jacobsona_-_Na_Stres_Niepok%C3%B3j_i_L%C4%99k_pgdtfq.mp4",
        thumbnail:
          "https://res.cloudinary.com/dxsukcxzj/image/upload/v1774467389/83d28776-985a-4b48-8afb-84a9a76d6482_t9zhkt.png",
      },
    ],
  },

  {
    title: "Relaks",
    data: [
      {
        id: "11",
        title: "Relaksacja z Intencją Miłości i Spokoju",
        videoUrl:
          "https://res.cloudinary.com/dxsukcxzj/video/upload/v1774468120/Relaksacja_na_Sen_i_na_Stres_1_qcgzti.mp4",
        thumbnail:
          "https://res.cloudinary.com/dxsukcxzj/image/upload/v1774467965/6_vtxwqk.jpg",
      },
      {
        id: "12",
        title: "Rozpuszaj stres i napięcie",
        videoUrl:
          "https://res.cloudinary.com/dxsukcxzj/video/upload/v1774468133/Relaksacja_rozpuszczaj%C4%85ca_stres_i_napi%C4%99cie_Medytacja_na_sen_i_relaks_x5ukgr.mp4",
        thumbnail:
          "https://res.cloudinary.com/dxsukcxzj/image/upload/v1774464493/tumblr_4faeb1ef8d94d259215b285bc4cadaba_8f2ade6b_500_revnxf.jpg",
      },
      {
        id: "13",
        title: "Medytacja oazy spokoju",
        videoUrl:
          "https://res.cloudinary.com/dxsukcxzj/video/upload/v1774468294/Medytacja_oazy_spokoju_ql9a7j.mp4",
        thumbnail:
          "https://res.cloudinary.com/dxsukcxzj/image/upload/v1774468325/R_2_cezklu.jpg",
      },
      {
        id: "14",
        title: "Medytacja na wewnętrzną siłę",
        videoUrl:
          "https://res.cloudinary.com/dxsukcxzj/video/upload/v1774468315/Medytacja_na_DOBRY_DZIE%C5%83_i_WEWN%C4%98TRZN%C4%84_SI%C5%81%C4%98_musbio.mp4",
        thumbnail:
          "https://res.cloudinary.com/dxsukcxzj/image/upload/v1774468104/R_1_kijyte.jpg",
      },
      {
        id: "15",
        title: "Wyłącz głowę przed snem",
        videoUrl:
          "https://res.cloudinary.com/dxsukcxzj/video/upload/v1774449689/Przenie%C5%9B_si%C4%99_do_INNEGO_%C5%9AWIATA_Medytacja_na_SEN_-_Chod%C5%BA_na_s%C5%82%C3%B3wko_720p_h264_def81g.mp4",
        thumbnail:
          "https://res.cloudinary.com/dxsukcxzj/image/upload/v1774449941/360_F_597640752_zRM6XcUtquOp3TXiCbln7zx2qbjsWDRx_dqztuo.jpg",
      },
    ],
  },

  {
    title: "Różne",
    data: [
      {
        id: "17",
        title: "Medytacja Uzdrawiająca",
        videoUrl:
          "https://res.cloudinary.com/dxsukcxzj/video/upload/v1774468719/Medytacja_Uzdrawiaj%C4%85ca_-_Medytacja_Prowadzona_dla_Wszystkich_ahzkls.mp4",
        thumbnail:
          "https://res.cloudinary.com/dxsukcxzj/image/upload/v1774469165/concept-of-conscious-living_djasxn.jpg",
      },
      {
        id: "18",
        title: "Medytacja na Nowe Początki ",
        videoUrl:
          "https://res.cloudinary.com/dxsukcxzj/video/upload/v1774468688/Medytacja_na_Nowe_Pocz%C4%85tki_i_Wewn%C4%99trzny_Spok%C3%B3j_plmhjy.mp4",
        thumbnail:
          "https://res.cloudinary.com/dxsukcxzj/image/upload/v1774469363/captivating-outdoor-landscape-view_1077802-38834_rjmqk3.jpg",
      },
      {
        id: "19",
        title: "Medytacja na chaos w głowie",
        videoUrl:
          "https://res.cloudinary.com/dxsukcxzj/video/upload/v1774464182/Medytacja_na_g%C5%82%C4%99boki_relaks_lub_sen_oddech_przeponowy_12_min_prowadzenia_bez_wybudzania_jnxipc.mp4",
        thumbnail:
          "https://res.cloudinary.com/dxsukcxzj/image/upload/v1774469261/develop-collection-of-illustrations-inspired-by-nature-showcasing-serene-landscapes-exotic-wildlife-and-lush-vegetation-photo_une7ns.jpg",
      },
      {
        id: "16",
        title: "Krótka medytacja na wewnętrzny spokój",
        videoUrl:
          "https://res.cloudinary.com/dxsukcxzj/video/upload/v1774468700/Kr%C3%B3tka_medytacja_na_WEWN%C4%98TRZNY_SPOK%C3%93J_ffrqls.mp4",
        thumbnail:
          "https://res.cloudinary.com/dxsukcxzj/image/upload/v1774468989/Zrzut_ekranu_2026-03-25_210256_znsvhx.png",
      },
      {
        id: "20",
        title: "Medytacja dla Wysoko Wrażliwych ",
        videoUrl:
          "https://res.cloudinary.com/dxsukcxzj/video/upload/v1774468797/Medytacja_dla_Wysoko_Wra%C5%BCliwych_Regeneracja_i_Granice_10_min_znj8h5.mp4",
        thumbnail:
          "https://res.cloudinary.com/dxsukcxzj/image/upload/v1774469315/image-145_aqrmow.jpg",
      },
    ],
  },
];

export default function BreathingScreen() {
  const [activeVideo, setActiveVideo] = useState<any | null>(null);

  // ✅ один player
  const player = useVideoPlayer(activeVideo?.videoUrl ?? "");

  return (
    <LayoutContainer>
      <SectionList
        sections={sections}
        keyExtractor={(item) => item.id}
        contentContainerStyle={{ padding: 20 }}
        ListHeaderComponent={
          <View style={styles.header}>
            <Header
              title={`Praktyki \n  Oddechowe`}
              image={require("../../../../assets/images/cloud.png")}
            />
          </View>
        }
        renderSectionHeader={({ section }) => (
          <Text style={styles.sectionTitle}>{section.title}</Text>
        )}
        renderItem={({ item: video }) => {
          const isActive = activeVideo?.id === video.id;

          return (
            <Pressable
              style={[styles.card, isActive && styles.activeCard]}
              onPress={() => setActiveVideo(isActive ? null : video)}
            >
              <View style={styles.video}>
                {isActive ? (
                  <VideoView
                    player={player}
                    style={StyleSheet.absoluteFillObject}
                    contentFit="cover"
                    allowsFullscreen
                  />
                ) : (
                  <>
                    <Image
                      source={{ uri: video.thumbnail }}
                      style={StyleSheet.absoluteFillObject}
                    />

                    <View style={styles.overlay} />

                    <View style={styles.playOverlay}>
                      <Text style={styles.play}>▶</Text>
                    </View>
                  </>
                )}
              </View>

              <Text numberOfLines={1} style={styles.videoTitle}>
                {video.title}
              </Text>
            </Pressable>
          );
        }}
      />
    </LayoutContainer>
  );
}

const styles = StyleSheet.create({
  header: {
    marginTop: -10,
    paddingHorizontal: 20,
  },

  section: {
    marginBottom: 24,
  },

  sectionTitle: {
    ...typography.title,
    color: colors.text.tertiary,
    paddingLeft: spacing.xs,
    marginBottom: spacing.md,
  },

  card: {
    width: 250,
    marginRight: 12,
  },

  activeCard: {
    transform: [{ scale: 0.95 }],
    borderRadius: 16,
  },

  video: {
    width: "100%",
    height: 150,
    borderRadius: 16,
    overflow: "hidden",
    justifyContent: "center",
    alignItems: "center",
  },

  overlay: {
    ...StyleSheet.absoluteFillObject,
    backgroundColor: "rgba(0,0,0,0.25)",
  },

  playOverlay: {
    ...StyleSheet.absoluteFillObject,
    justifyContent: "center",
    alignItems: "center",
  },

  play: {
    fontSize: 30,
    color: "white",
  },

  videoTitle: {
    marginTop: 6,
    ...typography.body,
    textAlign: "left",
    paddingLeft: spacing.xs,
    color: colors.text.secondary,
  },

  videoPlayer: {
    width: "100%",
    height: 220,
    borderRadius: 16,
    marginBottom: 20,
  },

  placeholder: {
    height: 220,
    borderRadius: 16,
    justifyContent: "center",
    alignItems: "center",
    backgroundColor: "#E5E7EB",
  },
});
